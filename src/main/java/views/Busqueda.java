...
public class Busqueda extends JFrame {
	...
	private void addReservaList(DefaultTableModel table, List<Reserva> reservas) {
		...
		for (Reserva reserva : reservas) {
			addReserva(reserva, table);
		}
	}

	public void addReserva(Reserva reserva, DefaultTableModel table) {
		Object[] rowData = { reserva.getId(), reserva.getFechaEntrada(), reserva.getFechaSalida(), reserva.getValor(),
				reserva.getFormaPago() };
		table.addRow(rowData);
	}

	private Reserva mapRowToReserva(int selectedRow) {
		Long id = ParseUtils.tryParseLong(modelo.getValueAt(selectedRow, 0).toString());
		Date fechaEntrada = DateUtils.parseStringToDate(modelo.getValueAt(selectedRow, 1).toString());
		Date fechaSalida = DateUtils.parseStringToDate(modelo.getValueAt(selectedRow, 2).toString());
		String formaPago = modelo.getValueAt(selectedRow, 4).toString();

		// Create a new Reserva object with the extracted values
	...
	private void addHuespedList(DefaultTableModel table, List<Huesped> huespedes) {
		...
		for (Huesped huesped : huespedes) {
			addHuesped(huesped, table);
		}
	}

	public void addHuesped(Huesped huesped, DefaultTableModel table) {
		Object[] rowData = { huesped.getId(), huesped.getNombre(), huesped.getApellido(), huesped.getFechaNacimiento(),
				huesped.getNacionalidad(), huesped.getTelefono(), huesped.getReserva().getId() };
		table.addRow(rowData);
	}

	private Huesped mapRowToHuesped(int selectedRow) {
		Long id = ParseUtils.tryParseLong(modeloHuesped.getValueAt(selectedRow, 0).toString());
